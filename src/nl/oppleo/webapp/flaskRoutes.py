...
#        return jsonify({ 'status': HTTP_CODE_401_UNAUTHORIZED, 'session': -1 if id is None else id, 'reason' : 'Authorization error' })
    ...
    fieldValidation = { 'start_value'       : r"^[0-9]*[,|.]?[0-9]?$",
                        'km'                : r"^[0-9]*$",
                        'end_value'         : r"^[0-9]*[,|.]?[0-9]?$",
                        'energy_device_id'  : r"^[0-9a-zA-Z!@#$%^&*._\-]+$",        #  r"^[0-9]|[a-z]|[A-Z]|[!@#$%\^&*._\-]+$",
                        'tariff'            : r"^(?:0|[1-9][0-9]*)(?:[.][0-9]{1,2})?$" 
                      }
...
def update_settings(param=None, value=None):
    ...
    if (param == 'chargerTariff' and
        ChargeSessionModel.has_open_charge_session_for_device(oppleoConfig.chargerID)):
        return jsonify({ 'status': 409, 'param': param, 'reason': 'Er is een laadsessie actief.' })
    validation = r"^(?:0|[1-9][0-9]*)(?:\.[0-9]{1,2})?$"
    ...
    # DATABASE_URL
    validation = r"^postgresql://[a-zA-Z0-9_\-]+:[^@]+@[a-zA-Z0-9_.\-:/]+$"
    ...
    # slave_address
    validation = r"^([01]?[0-9]?[0-9]|2[0-4][0-9]|25[0-5])$"
    ...
    validation = r"^(?!0+(?:\.0+)?$)(?:9|9.0|9.00|9.000|[0-9]{1}(?:\.[0-9]{1,3})?)$"
    ...
    # mqttHost
    validation = r"^(((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(\.|$|((\/(3[0-2]|[012]?[0-9])){0,1})$)){4})$|^((?=.{1,255}$)[0-9A-Za-z](?:(?:[0-9A-Za-z]|-){0,61}[0-9A-Za-z])?(?:\.[0-9A-Za-z](?:(?:[0-9A-Za-z]|-){0,61}[0-9A-Za-z])?)*\.?)$"
    if (param == 'mqttHost') and isinstance(value, str) and re.match(validation, value):
     ...
     # mqttPort
    validation = r"^([0-9]{1,4}|[1-5][0-9]{4}|6[0-4][0-9]{3}|65[0-4][0-9]{2}|655[0-2][0-9]|6553[0-5])$"
